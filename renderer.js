...
document.addEventListener("DOMContentLoaded", function () {
  ...
  sellerNIPInput.addEventListener("focus", () =>
    disableInputError(sellerNIPInput),
  );
  sellerNIPInput.addEventListener("blur", () =>
    handleNIPValidation(sellerNIPInput),
  );

  sellerPeselInput.addEventListener("focus", () =>
    disableInputError(sellerPeselInput),
  );
  sellerPeselInput.addEventListener("blur", () =>
    handlePeselValidation(sellerPeselInput),
  );

  customerPeselInput.addEventListener("focus", () =>
    disableInputError(customerPeselInput),
  );
  customerPeselInput.addEventListener("blur", () =>
    handlePeselValidation(customerPeselInput),
  );

  customerNIPInput.addEventListener("focus", () =>
    disableInputError(customerNIPInput),
  );
...
function isValidPesel(pesel) {
  ...
  if (10 - (peselSum % 10) == peselDigits[10]) return true;
  else return false;
}
...
async function sendSoapRequest(url, soapEnvelope, sessionId) {
  try {
    ...
    if (!navigator.onLine) alert("Brak połączenia z siecią internet!");
  }
}

function extractDataFromGUSResponse(responseString) {
  const replacedString = responseString
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/&#xD;/g, "");
  ...
  const name = replacedString.match(/<Nazwa>(.*?)<\/Nazwa>/),
    ...
    buildingNo = replacedString.match(
...
function insertInvoiceLine() {
  fetch("./html-elems/invoice-line.html")
    ...
    .then((htmlString) => {
      ...
      newline
        .querySelectorAll(".quan, .net-price, .vat-rate")
        .forEach((input) => {
      ...
      newline.querySelectorAll(".gross-amount").forEach((input) => {
        input.addEventListener("input", () =>
          updateRowOnGrossValueEdit(newline),
        );
      });
    })
    .catch((error) => {
      console.error(
        "There has been a problem with adding new invoice line:",
        error,
      );
    });
}
...
function deleteInvoiceLine(button) {
  ...
  if (!document.querySelector(".invoice-line")) insertInvoiceLine();
...
function updateRow(row) {
  ...
  row.querySelector("td .gross-price").value = ceil2digits(
    netPrice * (1 + vatRate / 100),
  );
  row.querySelector("td .gross-amount").value = ceil2digits(
    quan * netPrice * (1 + vatRate / 100),
  );
...
function updateRowOnNetValueEdit(row) {
  ...
  if (quan != 0) row.querySelector("td .net-price").value = netPrice;
  else row.querySelector("td .net-price").value = netValue;
...
function updateRowOnGrossValueEdit(row) {
  const grossValue =
    parseFloat(row.querySelector("td .gross-amount").value) || 0;
...
function updateSums() {
  ...
  document.getElementById("total-net-amount").value =
    ceil2digits(totalNetValue) + " " + currencyId;
  document.getElementById("total-gross-amount").value =
    ceil2digits(totalGrossValue) + " " + currencyId;
}
...
function amountToTextPLN(amount) {
  ...
  //Jeden
  if (złotych == 1) słownieZłotych += " złoty";
  //Naście
  ...
  else słownieZłotych += " złotych";

  const groszy = parseInt(100 * amount) - 100 * złotych;
  let słownieGroszy = amountToText(groszy);

  //Jeden
  if (groszy == 1) słownieGroszy += " grosz";
  //Naście
  else if (groszy % 100 > 10 && groszy % 100 < 20) słownieGroszy += " groszy";
  //Nie kilkanaście z końcówką dwa, trzy lub cztery
  else if (groszy % 10 == 2 || groszy % 10 == 3 || groszy % 10 == 4)
    słownieGroszy += " grosze";
  else słownieGroszy += " groszy";

  return słownieZłotych + " " + słownieGroszy;
}

async function saveLogoToLocalStorage(imageBlob = undefined) {
...
function generateInvoice() {
  ...
  fetch("./html-elems/invoice-print.html")
    ...
    .then((htmlString) => {
      ...
      invoiceLinesHeader.appendChild(
        document.querySelector("#invoice-lines-table thead tr").cloneNode(true),
      );
      ...
      let lineCount = 1;
      document
        .querySelectorAll("#invoice-lines-table tbody tr")
        .forEach((line) => {
      ...
      const upfrontPaymentAmount = document.getElementById(
        "upfront-payment-amount",
      );

      if (upfrontPaymentAmount > 0) {
        var amountDue = totalGrossAmount - upfrontPaymentAmount;
        invoice.getElementById("upfront-payment-amount").innerHTML =
      ...
      invoice.getElementById("amount-due").innerHTML =
        parseFloat(amountDue).toFixed(2) +
        " " +
        document.getElementById("curr-id").value;
      invoice.getElementById("amount-due-text").innerHTML =
        amountToTextPLN(amountDue);

      //Additional data
      if (document.getElementById("no-vat").checked)
        invoice.getElementById("no-vat").innerHTML =
          "Podstawa zwolnienia z podatku VAT: Zwolnienie podmiotowe zg. z art. 113 ust. 1 i 9 ustawy o VAT.";
      ...
      if (document.getElementById("seller-company-btn").checked) {
        sellerNipOrPesel.innerHTML =
          "NIP: " + document.getElementById("seller-nip").value;
        sellerName.innerHTML = document.getElementById(
          "seller-company-name",
        ).value;
      } else if (document.getElementById("seller-person-btn").checked) {
        sellerNipOrPesel.innerHTML =
          "PESEL: " + document.getElementById("seller-pesel").value;
        sellerName.innerHTML =
          document.getElementById("seller-person-name").value;
      }
      ...
      if (document.getElementById("customer-company-btn").checked) {
        customerNipOrPesel.innerHTML =
          "NIP: " + document.getElementById("customer-nip").value;
        customerName.innerHTML = document.getElementById(
          "customer-company-name",
        ).value;
      } else if (document.getElementById("customer-person-btn").checked) {
        customerNipOrPesel.innerHTML =
          "PESEL: " + document.getElementById("customer-pesel").value;
        customerName.innerHTML = document.getElementById(
      ...
      window.main.openInvoicePrint(
        new XMLSerializer().serializeToString(invoice),
      );
    })
    .catch((error) => {
      console.error(
        "There has been a problem with generating invoice printable:",
        error,
      );
    });
}
...
async function getDataFromGUS(NIP) {
  ...
  await fetch("./env.json")
    ...
    .then((json) => {
      ...
      } else throw new Error("Unknown or empty STATUS environmental variable.");
    });

  //Preparing API login SOAP envelope
  let loginXml = await fetch("./soap-envelopes/login.xml")
    .then((response) => {
      if (!response.ok) throw new Error("File: login.xml not found!");
  ...
  const sessionId = await sendSoapRequest(apiAddr, loginXml).then(
    (response) => {
  ...
  let nipLookupXml = await fetch("./soap-envelopes/nip-lookup.xml")
    .then((response) => {
      if (!response.ok) throw new Error("File: nip-lookup.xml not found!");
  ...
  let companyData = await sendSoapRequest(
    apiAddr,
    nipLookupXml,
    sessionId,
  ).then((response) => {
  ...
  fetch("./soap-envelopes/logout.xml")
    .then((response) => {
      if (!response.ok) throw new Error("File: logout.xml not found!");
